...
void SetVertexInputAttributeFormat(context *C, object *Object, int IsCurrent, u32 Index, GLint Size, GLenum Type, GLuint RelativeOffset, u32 IntegerHandlingBits, const char *Name) {
    CheckGL(Index >= C->PipelineStatesTypes[pipeline_state_VERTEX_INPUT_ATTRIBUTES].InstanceCount, gl_error_VERTEX_ARRAY_ATTRIB_FORMAT_INDEX);
    ...
    pipeline_state_vertex_input_attribute *State = Object->VertexArray.InputAttributes + Index;
    State->Size = Size;
    State->Type = Type;
    ...
    if(IsCurrent) {
        State = GetCurrentPipelineState(C, pipeline_state_VERTEX_INPUT_ATTRIBUTES);
        State += Index;
        State->Size = Size;
        State->Type = Type;
        State->RelativeOffset = RelativeOffset;
...
void SetVertexInputBinding(context *C, object *Object, int IsCurrent, u32 Index, GLuint Vbo, GLintptr Offset, GLsizei Stride, const char *Name) {
    CheckGL(Index >= C->PipelineStatesTypes[pipeline_state_VERTEX_INPUT_BINDINGS].InstanceCount, gl_error_VERTEX_ARRAY_VERTEX_BUFFER_INDEX);
    ...
    pipeline_state_vertex_input_binding NewState = {0};
    if(Vbo != 0) {
        NewState.Vbo = Vbo;
        NewState.Offset = Offset;
        NewState.Stride = Stride;
    }

    Object->VertexArray.InputBindings[Index] = NewState;
    if(IsCurrent) {
...
void SetVertexInputAttributeBinding(context *C, object *Object, int IsCurrent, u32 AttributeIndex, u32 BindingIndex, const char *Name) {
    CheckGL(AttributeIndex >= C->PipelineStatesTypes[pipeline_state_VERTEX_INPUT_ATTRIBUTES].InstanceCount, gl_error_VERTEX_ARRAY_ATTRIB_BINDING_ATTRIB_INDEX);
    CheckGL(BindingIndex >= C->PipelineStatesTypes[pipeline_state_VERTEX_INPUT_BINDINGS].InstanceCount, gl_error_VERTEX_ARRAY_ATTRIB_BINDING_BINDING_INDEX);
...
void SetVertexInputAttributeEnabled(context *C, object *Object, int IsCurrent, GLuint Index, int IsEnabled, const char *Name) {
    CheckGL(Index >= C->PipelineStatesTypes[pipeline_state_VERTEX_INPUT_ATTRIBUTES].InstanceCount, gl_error_XABLE_VERTEX_ATTRIB_INDEX);
...
int CreatePipelineStates(context *C, const VkPhysicalDeviceLimits *Limits) {
    SetPipelineStateInfos(C->PipelineStatesTypes, Limits, &C->PipelineStateByteCount);

    return ArrayRequireRoom(&C->PipelineStates, 2, C->PipelineStateByteCount, INITIAL_PIPELINE_STATE_CAPACITY);
}
...
void *GetPipelineState(context *C, u64 StateIndex, pipeline_state_type Type) {
    ...
    return Base + C->PipelineStatesTypes[Type].ByteOffset;
}

void ClearPipelineState(context *C, u64 StateIndex, pipeline_state_type Type) {
    pipeline_state_info Info = C->PipelineStatesTypes[Type];
